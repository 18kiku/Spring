			...
			<hr class="d_line">
						...
						<c:forEach var="cart" items="${cartList }">
							<form action="cartUpdate.do" method="post" name="cartForm">
								...
								<tr>
									...
									<a href="shopDetail?product_id=${cart.product_id }"><img src="/spring_images/${cart.product_image }" width="60" height="60"></a></td>
									<td class="left td3" width="48%">
										<span class="s1">
											<a href="shopDetail.do?product_id=${cart.product_id}">${cart.product_name }</a>
										</span><br>
										<span class="s2">
											${cart.product_size} | ${product.product_color }
										</span><br>
										<span class="s3">${cart.product_price }원</span> | 
										<span class="s4"><fmt:formatNumber pattern="#,###,###" value="${cart.product_price * cart.discount_rate / 100}"/>원</span> 
										(<span class="s5">${cart.discount_rate }%할인</span>)
									</td>
									...
									<td class="right td5" width="9%"><fmt:formatNumber pattern="#,###,###" value="${cart.product_price * cart.discount_rate / 100}"/>원</td>
				...
				<table class="t_cart_tot">
					<tr>
						...
						<th><img src="resources/img/plus.PNG" width="30"></th>
						<th>총 추가금액<br><span class="s2"><%=0 %></span>원</th>
						<th><img src="resources/img/minus.PNG" width="30"></th>
						<th>총 할인금액<br><span class="s3"><%=0 %></span>원</th>
						<th><img src="resources/img/equal.PNG" width="30"></th>
				...
				<table class="t_cart_address">
					<tr>
						...
						<td>배송지 : <span id="address">${member.address }&ensp;${member.address_detail }</span> <input type="button" value="배송지 변경" id="btn_address"></td>
					</tr>
				</table>
				<div class="d5">
